-- ============================================================
-- 04-spatial-seed.sql
-- Council Agent Portal — Manningham City Council spatial data
-- Run AFTER 03-spatial-ddl.sql
-- All coordinates are WGS84 (EPSG:4326) for Manningham, VIC
-- ============================================================
...
WHERE NOT EXISTS (SELECT 1 FROM spatial_features sf JOIN spatial_layers sl ON sf.layer_id = sl.layer_id WHERE sl.code = 'facilities' LIMIT 1);
...
WHERE NOT EXISTS (SELECT 1 FROM spatial_features sf JOIN spatial_layers sl ON sf.layer_id = sl.layer_id WHERE sl.code = 'parks' LIMIT 1);
...
WHERE NOT EXISTS (SELECT 1 FROM spatial_features sf JOIN spatial_layers sl ON sf.layer_id = sl.layer_id WHERE sl.code = 'infrastructure' LIMIT 1);
...
SELECT
    ...
    v.name, v.description, v.props::jsonb,
    ST_SetSRID(ST_GeomFromText(v.wkt), 4326)
FROM (VALUES
  ('Koonung Ward',
...
WHERE NOT EXISTS (SELECT 1 FROM spatial_features sf JOIN spatial_layers sl ON sf.layer_id = sl.layer_id WHERE sl.code = 'wards' LIMIT 1);
...
WHERE NOT EXISTS (SELECT 1 FROM spatial_features sf JOIN spatial_layers sl ON sf.layer_id = sl.layer_id WHERE sl.code = 'trees' LIMIT 1);
...
WHERE NOT EXISTS (SELECT 1 FROM spatial_features sf JOIN spatial_layers sl ON sf.layer_id = sl.layer_id WHERE sl.code = 'zones' LIMIT 1);
